...
function showMigrationModal(okCallback: () => Promise<void>, cancelCallback: () => Promise<void>) {
    showModal(
        ...
        >
            TabMaster now saves settings and tabs for each user of the device. Would you like to transfer this device's
            previous settings to your account?
        </ConfirmModal>
    )
}
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (hadLegacySettings instanceof Error) {
                LogController.raiseError(
                    'TabMaster encountered a problem during initialization process.',
                    `PythonInterop.setActiveSteamId returned error: "${hadLegacySettings.message}"`
                )
                resolve()
                return
            }
