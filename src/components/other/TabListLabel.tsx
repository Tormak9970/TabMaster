    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', width: '100%', ...style }}>
            ...
            {tabContainer.filters ? (
                (tabContainer as CustomTabContainer).dependsOnMicroSDeck ? (
                    <FaSdCard fill={microSDeckDisabled ? '#92939b61' : 'currentColor'} />
                ) : (
                    <Fragment />
                )
            ) : (
            ...
            {tabContainer.position !== -1 &&
                ...
                (tabContainer as CustomTabContainer).collection.visibleApps.length === 0 && (
                    <BiSolidHide style={{ marginLeft: 'auto' }} />
                )}
        </div>
    )
}
