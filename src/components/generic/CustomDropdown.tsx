...
export interface CustomDropdownProps
    extends
        Omit<
            DropdownProps,
            'rgOptions' | 'onMenuWillOpen' | 'selectedOption' | 'contextMenuPositionOptions' | 'renderButtonValue'
        >,
        Omit<CustomButtonProps, 'audioSFX' | 'noAudio' | 'onClick' | 'children'> {
    ...
    const icon =
        ...
        (CustomModal ? (
            ...
            <svg
                ...
                fill='none'
            >
                <path d='M17.98 26.54L3.20996 11.77H32.75L17.98 26.54Z' fill='currentColor'></path>
            </svg>
        ))
    const [selected, setSelected] = useState<SingleDropdownOption | undefined>(
        rgOptions?.find(option => option.data === selectedOptionData)
    )
        ...
        >
            <div style={{ display: 'flex', overflow: 'hidden' }}>
                <div style={{ overflow: 'hidden', flex: 'auto' }}>
                    <div
                        style={Object.assign(
                            { textAlign: labelCenter ? 'center' : 'left', minHeight: '20px' },
                            changed ? labelChangedStyle : labelStyle
                        )}
                        className={addClasses(
                            CustomDropdownClasses.label,
                            changed && CustomDropdownClasses.selectionChanged
                        )}
                    >
                        {labelOverride ?? selected?.label ?? strDefaultLabel}
                    </div>
                </div>
                {!noDropdownIcon && <div style={{ display: 'flex', marginLeft: '1ch', flex: 'none' }}>{icon}</div>}
            </div>
        </CustomButton>
    )
}
