...
export const ListSearchModal: VFC<ListSearchModalProps> = ({
    rgOptions: list,
    entryLabel,
    determineEntryIcon,
    onSelectOption,
    closeModal,
}: ListSearchModalProps) => {
    ...
    const ListEntry = useMemo<(props: { index: number; style: any }) => JSX.Element>(
        () =>
            ({ index, style }) => {
                    ...
                    <DialogControlsSection style={{ marginTop: '1em' }}>
                        <div style={{ width: '100%', position: 'relative' }}>
                            ...
                            <div className='more-below-arrow'>
                                {renderBottomArrow && (
                                    <BiSolidDownArrow style={{ fontSize: '0.8em', color: '#343945' }} />
                                )}
                            </div>
                        </div>
                    </DialogControlsSection>
...
export function ListSearchTrigger({
    entryLabel,
    labelOverride,
    options,
    onChange,
...
export function ListSearchDropdown({
    entryLabel,
    rgOptions,
    selectedOption,
    onChange,
    ...
    determineEntryIcon,
    disabled,
}: ListSearchDropdownProps) {
    const [selected, setSelected] = useState<SingleDropdownOption>(
        rgOptions.find((option: SingleDropdownOption) => option.data === selectedOption)!
    )
