import { afterPatch, findInReactTree, Patch, replacePatch, showContextMenu, wrapReactType } from '@decky/ui'
            ...
            afterPatch(ret1, 'type', (_: Record<string, unknown>[], ret2: ReactElement) => {
                ...
                if (memoCache) {
                    ...
                    innerPatch = replacePatch(ret2.type, 'type', args => {
                        const hooks =
                            (window.SP_REACT as any)?.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED
                        ...
                        const fakeUseMemo = (fn: () => any, deps: any[]) => {
                            return realUseMemo(() => {
                                ...
                                const isNested = Array.isArray(original) && Array.isArray(original[0])
                                const tabs: SteamTab[] = isNested ? original[0] : original
                                ...
                                const collectionsAppFilterGamepad = deps.find(
                                    (dep: any) => typeof dep?.Matches === 'function'
                                )

                                if (collectionsAppFilterGamepad === undefined) {
                                    LogController.raiseError(`Couldn't find collection filter function to copy from`)
                                    return original
                                }
                                ...
                                const TabAppGrid =
                                    ...
                                    findInReactTree(
                                        tabTemplate.content,
                                        elt => elt.type && elt.type.toString?.().includes('Library_FilteredByHeader')
                                    )?.type
                                            ...
                                            return (
                                                tabs.find(actualTab => {
                                                    if (actualTab.id === tabContainer.id) {
                                                        ...
                                                        actualTab.footer.onMenuButton = getShowMenu(
                                                            tabContainer.id,
                                                            tabMasterManager
                                                        )
                                                        return true
...
function getShowMenu(id: string, tabMasterManager: TabMasterManager) {
    return () => {
        let menu: { Hide: () => void }
        const menuElement = (
            <LibraryMenu selectedTabId={id} tabMasterManager={tabMasterManager} closeMenu={() => menu.Hide()} />
        )
        //@ts-ignore
